...
class DataSet:
    ...
    def __init__(
        ...
        self.output_folder.joinpath("img/temporal_features").mkdir(
            parents=True, exist_ok=True
        )
        self.output_folder.joinpath("img/data_overview").mkdir(
            parents=True, exist_ok=True
        )
        self.output_folder.joinpath("img/features_analysis").mkdir(
            parents=True, exist_ok=True
        )
        self.output_folder.joinpath("img/spatial_features").mkdir(
            parents=True, exist_ok=True
        )
        ...
        self.deployments_created = list(
            self.output_folder.joinpath("deployments").glob("*.nc")
        )
        self.dataset = {}

        self.survey_dependent_attrs = [
            "hydrophone_name",
            "hydrophone_model",
            "hydrophone_Vpp",
            "hydrophone_preamp_gain",
    ...
    def generate_deployment(self, idx: int) -> xarray.Dataset:
        ...
        if self.temporal_features not in [[], None]:
            temporal_evo = asa.evolution_multiple(
                method_list=self.temporal_features, band_list=self.band_list
            )
        ...
        self.metadata.loc[
            idx, "end_to_end_calibration"
        ] = hydrophone.end_to_end_calibration(p_ref=1.0)
        self.metadata.to_csv(self.summary_path, index=False)
        return ds
    ...
    def add_deployment_metadata(self, idx: int) -> None:
        ...
        asa = acoustic_survey.ASA(
            hydrophone=hydrophone, folder_path=deployment_row["folder_path"]
        )
            ...
            for feature in self.temporal_features:
                ...
                plt.savefig(
                    self.output_folder.joinpath(
                        "img/temporal_features/%s_%s_%s.png"
                        % (i, station_name, feature)
                    )
                )
                plt.show()
                i += 1
    ...
    def plot_third_octave_bands_prob(
        ...
        bin_edges = np.arange(
            start=self.dataset["oct3"].min().min(),
            stop=self.dataset["oct3"].max().max(),
            step=h,
        )
        ...
        for station_name, deployment in self.dataset.items():
            ...
            fig = plt.figure()
            im = plt.pcolormesh(
                fbands, bin_edges[:-1], spd.T, cmap="BuPu", shading="auto"
            )
            ...
            plt.savefig(
                self.output_folder.joinpath(
                    "img/features_analysis/%s_%s_third_oct_prob.png"
    ...
    def plot_third_octave_bands_avg(self) -> None:
        ...
        for station_name, deployment in self.dataset.items():
            ...
            plt.savefig(
                self.output_folder.joinpath(
                    "img/features_analysis/%s_%s_third_oct_avg.png"
                    % (station_i, station_name)
                )
            )
            plt.show()
