...
acu_file = AcuFile(
    sfile=f"{test_dir}/test_data/67416073.210610033655.wav",
...
class TestAcousticIndices(unittest.TestCase):
    ...
    def test_compute_bi(self):
        bi = pypam.acoustic_indices.compute_bi(
            sxx=self.spectrogram, frequencies=self.frequencies
        )
        assert bi >= 0
    ...
    def test_compute_ndsi(self):
        ndsi = pypam.acoustic_indices.compute_ndsi(
            sxx=self.spectrogram, frequencies=self.frequencies
        )
        assert np.logical_and(ndsi >= -1, ndsi <= 1)

    def test_compute_aei(self):
        aei = pypam.acoustic_indices.compute_aei(
            sxx=self.spectrogram, frequencies=self.frequencies
        )
        assert np.logical_and(aei >= 0, aei <= 1)

    def test_compute_adi(self):
        adi = pypam.acoustic_indices.compute_adi(
            sxx=self.spectrogram, frequencies=self.frequencies
        )
        assert adi >= 0

    def test_compute_zcr(self):
        zcr = pypam.acoustic_indices.compute_zcr(
            s=self.signal.signal, fs=self.signal.fs
        )
        assert np.logical_and(zcr >= 0, zcr <= 1)

    def test_compute_zcr_avg(self):
        zcr_avg = pypam.acoustic_indices.compute_zcr_avg(
            s=self.signal.signal, fs=self.signal.fs
        )
