...
def test_get_millidecade_bands(artificial_data):
    ...
    mdec_bands_test = pd.read_csv(f'{test_dir}//test_data/mdec_bands_test.csv', header=None)
...
def test_energy_window():
    ...
    def load_and_window_data(filename, plot=False):
        ...
        start,end = utils.energy_window(P_interp,.97)
        tau_97 = t_interp[end]-t_interp[start]
        start,end = utils.energy_window(P_interp,.90)
