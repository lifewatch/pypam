...
def artificial_data():
    ...
    noise_amp = 100
    signal_amp = 100
    data = pd.read_csv(f"{test_dir}/test_data/signal_data.csv", header=None)[0].values
...
def test_psd_to_millidecades(artificial_data):
    ...
    spectra_manta = pd.read_csv(f"{test_dir}//test_data/spectra.csv", header=None)
    ...
    psd_da = xarray.DataArray(
        [spectra], coords={"id": [0], "frequency": fbands}, dims=["id", "frequency"]
    )
    ...
    assert (
        (mdec_power_test["sum"] - milli_psd_power.sel(id=0).values).abs() > 1e-5
    ).sum() == 0
...
def test_kurtosis():
    ...
    result_scipy = scipy.stats.kurtosis(normally_distributed_data, fisher=False)
...
def test_energy_window():
    ...
    def load_and_window_data(filename, plot=False):
        raw_data = np.loadtxt(
            f"{test_dir}/test_data/impulsive_data/{filename}", delimiter=","
        )
        ...
        if plot:
            ...
            ax.plot(t_interp[start:end], P_interp[start:end], color="r", linestyle="--")
            fig.suptitle(f"energy window test, Madsen 2005, tau_90={tau_90:.2f} us")
            plt.show()
